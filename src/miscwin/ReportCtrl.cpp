...
// be redistributed unmodified by any means PROVIDING it is not sold for profit
// without the authors written consent, and providing that this notice and the
// authors name is included. If the source code in  this file is used in any
// commercial application then acknowledgement must be made to the author of
// this file .
//
...
// Dec. 03, 2003 - Fixed a bug in "EndEdit" where item text were not preperly
// committed.
//               - Completed the implementation of the "Sort-Separator" feature.
// Jan. 01, 2004 - Fixed a bug in "SetItemData".
//               - Added message "WM_EDIT_COMMITTED" which is sent to the parent
//               window
//                 when an item text editing is committed.
//               - Fixed a bug in "SetItemText"(double type).
//               - Fixed a bug where item sorting does not work properly when
//               there
...
int CReportCtrl::InsertItem(UINT nMask, int nItem, LPCTSTR lpItem, UINT nState,
...
int CReportCtrl::InsertColumn(int nCol, LPCTSTR lpColumnHeading, int nFormat,
...
int CReportCtrl::MoveToTop(int nItem) { return MoveTo(nItem, -1); }

int CReportCtrl::MoveToBottom(int nItem) { return MoveTo(nItem, INT_MAX); }
...
BOOL CReportCtrl::SetItemText(int nItem, int nSubItem, DOUBLE val,
                              int nPrecision) {
  CString sText, sFmt;
...
BOOL CReportCtrl::SetItemText(int nItem, int nSubItem,
...
LPCTSTR CReportCtrl::GetSortSeparator() const { return m_pszSeparator; }
...
BOOL CReportCtrl::IsEditable() const { return m_bAllowEdit; }

BOOL CReportCtrl::_ItemCheckMonitor(int nIndex, BOOL bBefore, BOOL bAfter,
                                    UINT nMsg) {
...
int CReportCtrl::GetCheckboxStyle() const { return m_nChkStyle; }

void CReportCtrl::_StringSplit(const CString& str, CStringArray& arr,
                               TCHAR chDelimitior) {
  int nStart = 0, nEnd = 0;
  arr.RemoveAll();
...
CEdit* CReportCtrl::GetEditControl() { return m_pWndEdit; }

BOOL CReportCtrl::Create(CWnd* pParentWnd, UINT nID, LPCRECT lpRect,
                         DWORD dwStyle) {
  // TODO: Add your specialized code here and/or call the base class
...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point,
                                   BOOL bTriggerEdit) {
  LVHITTESTINFO hti;
  hti.pt = point;
...
int CReportCtrl::GetSortedColumn() const { return m_nSortCol; }

BOOL CReportCtrl::IsSortAscending() const { return m_bSortAscending; }
...
BOOL CReportCtrl::SetItemData(int nItem, LPARAM dwData) {
  CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
...
LPARAM CReportCtrl::GetItemData(int nItem) const {
  CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
...
BOOL CReportCtrl::SetItem(int nItem, int nSubItem, UINT nMask, LPCTSTR lpszItem,
...
void CReportCtrl::SetItemTextColor(int nItem, int nSubItem, COLORREF color,
                                   BOOL bRedraw) {
...
void CReportCtrl::SetItemBkColor(int nItem, int nSubItem, COLORREF color,
                                 BOOL bRedraw) {
...
int CReportCtrl::FindItem(LVFINDINFO* pFindInfo, int nStart) const {
