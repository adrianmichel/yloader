...
// ReportCtrl.cpp
//
// CYReportCtrl, a CListCtrl derived class that is specialized on "Report View"
// style. 
//
// Features:
//
...
//                 are multiple CYReportCtrl objects on same window.
//
///////////////////////////////////////////////////////////////////////////////
...
#define MUST_NOT_STYLE \
  (LVS_EDITLABELS | LVS_ICON | LVS_SMALLICON | LVS_LIST | LVS_NOSCROLL)
...
class CItemData {
 public:
  ...
  LPARAM dwData;  // The actual 32-bit user data stores here
...
int _ITEM_COMPARE_FUNCS::_DateCompare(const COleDateTime& date1,
...
/////////////////////////////////////////////////////////////////////////////
// CYReportCtrl Implementation
/////////////////////////////////////////////////////////////////////////////
::CYReportCtrl::CYReportCtrl() : m_pWndEdit(NULL) {
...
::CYReportCtrl::~CYReportCtrl() {
  if (m_pszSeparator != NULL) delete[] m_pszSeparator;

  if (m_pWndEdit != NULL) delete m_pWndEdit;
}

BEGIN_MESSAGE_MAP(CYReportCtrl, CListCtrl)
//{{AFX_MSG_MAP(CYReportCtrl)
...
/////////////////////////////////////////////////////////////////////////////
// CYReportCtrl message handlers
void ::CYReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult) {
  ...
  if (lplvcd->nmcd.dwDrawStage == CDDS_PREPAINT) {
    ...
    *pResult = CDRF_NOTIFYSUBITEMDRAW;
    ...
    CItemData* p = reinterpret_cast<CItemData*>(
        CListCtrl::GetItemData(lplvcd->nmcd.dwItemSpec));
    ASSERT(p != NULL);
    ASSERT(lplvcd->iSubItem >= 0 &&
           lplvcd->iSubItem < p->aTextColors.GetSize());
...
void CYReportCtrl::OnColumnclick(NMHDR* pNMHDR, LRESULT* pResult) {
...
void CYReportCtrl::OnLButtonDown(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_LBUTTONDOWN, nFlags, point, TRUE);
}

void CYReportCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_LBUTTONDBLCLK, nFlags, point, TRUE);
}

void CYReportCtrl::OnMButtonDown(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_MBUTTONDOWN, nFlags, point, FALSE);
}

void CYReportCtrl::OnMButtonDblClk(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_MBUTTONDBLCLK, nFlags, point, FALSE);
}

void CYReportCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_RBUTTONDOWN, nFlags, point, FALSE);
}

void CYReportCtrl::OnRButtonDblClk(UINT nFlags, CPoint point) {
  // TODO: Add your message handler code here and/or call default
  _MouseClkMonitor(WM_RBUTTONDBLCLK, nFlags, point, FALSE);
}

int CYReportCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) {
...
BOOL CYReportCtrl::_IsValidIndex(int nIndex) const {
  return nIndex >= 0 && nIndex < CListCtrl::GetItemCount();
}

// the heading text is in the format of
// "text,width,format;text,width,format;..."
BOOL CYReportCtrl::SetColumnHeader(const CString& strHeadings) {
  DeleteAllItems();
  DeleteAllColumns();
  EndEdit(TRUE);
  ...
  for (int i = 0; i < aLong.GetSize(); i++) {
    ...
    if (aShort.GetSize() > 0) {
      ...
      bInserted |=
          (InsertColumn(GetColumnCount(), aShort[0], nFormat, WIDTH) >= 0);
    }
  }
  return bInserted;
}

int CYReportCtrl::InsertItem(int nIndex, LPCTSTR lpText) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
...
BOOL CYReportCtrl::DeleteItem(int nItem, BOOL bSelectNextItem) {
  EndEdit(m_ptEditting.x != nItem);

  if (bSelectNextItem) SetItemStates(nItem + 1, RC_ITEM_SELECTED);

  _FreeItemMemory(nItem);
  return CListCtrl::DeleteItem(nItem);
}

int CYReportCtrl::DeleteAllItems(DWORD dwStates) {
  EndEdit(FALSE);
  int nItemCount = CListCtrl::GetItemCount();
  if (dwStates & RC_ITEM_ALL) {
    LockWindowUpdate();
    for (int i = 0; i < nItemCount; i++) _FreeItemMemory(i);
...
void CYReportCtrl::SortItems(int nColumn, BOOL bAscending) {
  EndEdit(TRUE);
  if (nColumn < 0 || nColumn >= GetColumnCount() || !IsSortable()) return;
  ...
  GetParent()->SendMessage(WM_ITEM_SORTED, (WPARAM)m_nSortCol,
                           (LPARAM)m_bSortAscending);
}

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, LPCTSTR lpText) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
  return CListCtrl::SetItemText(nItem, nSubItem, lpText);
}

int CYReportCtrl::GetColumnCount() const {
  return GetHeaderCtrl()->GetItemCount();
}

int CYReportCtrl::InsertItem(const LVITEM* pItem) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
...
int CYReportCtrl::InsertItem(int nItem, LPCTSTR lpItem, int nImage) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
...
int CYReportCtrl::InsertItem(UINT nMask, int nItem, LPCTSTR lpItem, UINT nState,
                             UINT nStateMask, int nImage, LPARAM lParam) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
  const int IDX = CListCtrl::InsertItem(nMask, nItem, lpItem, nState,
                                        nStateMask, nImage, lParam);
  if (IDX >= 0) _AllocItemMemory(IDX);
  return IDX;
}

void CYReportCtrl::SetGridLines(BOOL bSet) {
...
int CYReportCtrl::InsertColumn(int nCol, const LVCOLUMN* pColumn) {
  EndEdit(TRUE);
  const int IDX = CListCtrl::InsertColumn(nCol, pColumn);
  if (IDX >= 0) _UpdateColumn(IDX, TRUE);
  return IDX;
}

int CYReportCtrl::InsertColumn(int nCol, LPCTSTR lpColumnHeading, int nFormat,
                               int nWidth, int nSubItem) {
  EndEdit(TRUE);
  const int IDX =
      CListCtrl::InsertColumn(nCol, lpColumnHeading, nFormat, nWidth, nSubItem);
  if (IDX >= 0) _UpdateColumn(IDX, TRUE);
  return IDX;
}

BOOL CYReportCtrl::DeleteColumn(int nCol) {
  EndEdit(TRUE);
  const BOOL RES = CListCtrl::DeleteColumn(nCol);
  if (RES) _UpdateColumn(nCol, FALSE);
  return RES;
}

void CYReportCtrl::SetCheckboxeStyle(int nStyle) {
  m_nChkStyle = nStyle;
  EndEdit(TRUE);
  DWORD dwStype = GetExtendedStyle();
  if (nStyle == 0) {
...
int CYReportCtrl::MoveTo(int nItem, int nNewPosition) {
  if (!_IsValidIndex(nItem)) return -1;
  ...
  const LPARAM DATA = CListCtrl::GetItemData(nItem);
  CArray<int, int> aImages;
  CStringArray aTexts;
...
int CYReportCtrl::MoveUp(int nItem, int nCount) {
  return MoveTo(nItem, nItem - nCount);
}

BOOL CYReportCtrl::MoveDown(int nItem, int nCount) {
  return MoveTo(nItem, nItem + nCount);
}

void CYReportCtrl::SwapItems(int nItem1, int nItem2) {
  EndEdit(TRUE);
  if (!_IsValidIndex(nItem1) || !_IsValidIndex(nItem2)) return;  // FALSE;

  if (nItem1 == nItem2) return;  // TRUE;
  ...
  const LPARAM DATA1 = CListCtrl::GetItemData(nItem1);
  const LPARAM DATA2 = CListCtrl::GetItemData(nItem2);
...
int CYReportCtrl::MoveToTop(int nItem) { return MoveTo(nItem, -1); }

int CYReportCtrl::MoveToBottom(int nItem) { return MoveTo(nItem, INT_MAX); }

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, TCHAR val) {
  return SetItemText(nItem, nSubItem, CString(val));
}

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, INT val) {
  return SetItemText(nItem, nSubItem, (LONG)val);
}

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, UINT val) {
  return SetItemText(nItem, nSubItem, (ULONG)val);
}

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, DOUBLE val,
                               int nPrecision) {
  CString sText, sFmt;
...
BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem,
...
BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, LONG val) {
  CString str;
  str.Format(_T("%d"), val);
  return SetItemText(nItem, nSubItem, str);
}

BOOL CYReportCtrl::SetItemText(int nItem, int nSubItem, ULONG val) {
  CString str;
  str.Format(_T("%u"), val);
  return SetItemText(nItem, nSubItem, str);
}

BOOL CYReportCtrl::DeleteAllColumns() {
  while (GetColumnCount() > 0) DeleteColumn(0);
  return TRUE;
}

LPCTSTR CYReportCtrl::GetSortSeparator() const { return m_pszSeparator; }

void CYReportCtrl::SetSortSeparator(LPCTSTR lpSortSeparator) {
  if (m_pszSeparator != NULL) {
    delete[] m_pszSeparator;
    m_pszSeparator = NULL;
  }

  if (lpSortSeparator != NULL) m_pszSeparator = _tcsdup(lpSortSeparator);
}

int CYReportCtrl::InsertItemEx(int nItem, LPCTSTR lpText, ...) {
  EndEdit(TRUE);
  _UnsetSortedColumn();
...
BOOL CYReportCtrl::_IsEditVisible() const {
  return m_bAllowEdit && m_ptEditting.x >= 0 &&
         m_ptEditting.x < CListCtrl::GetItemCount() && m_ptEditting.y >= 0 &&
         m_ptEditting.y < GetColumnCount();
}

void CYReportCtrl::SetEditable(BOOL bSet) {
  if (!bSet) EndEdit(TRUE);
  m_bAllowEdit = bSet;
}

BOOL CYReportCtrl::IsEditable() const { return m_bAllowEdit; }

BOOL CYReportCtrl::_ItemCheckMonitor(int nIndex, BOOL bBefore, BOOL bAfter,
                                     UINT nMsg) {
  if (!_IsValidIndex(nIndex) || m_nChkStyle == RC_CHKBOX_NONE ||
      (bBefore && bAfter) || (!bBefore && !bAfter)) {
    return FALSE;
  }
...
int CYReportCtrl::GetCheckboxStyle() const { return m_nChkStyle; }

void CYReportCtrl::_StringSplit(const CString& str, CStringArray& arr,
                                TCHAR chDelimitior) {
  int nStart = 0, nEnd = 0;
  arr.RemoveAll();
...
BOOL CYReportCtrl::SetSortable(BOOL bSet) {
...
DWORD CYReportCtrl::SetExtendedStyle(DWORD dwNewStyle) {
  dwNewStyle &= ~MUST_NOT_EX_STYLE;
  dwNewStyle |= MUST_EX_STYLE;
  return CListCtrl::SetExtendedStyle(dwNewStyle);
}

BOOL CYReportCtrl::PreCreateWindow(CREATESTRUCT& cs) {
...
BOOL CYReportCtrl::PreTranslateMessage(MSG* pMsg) {
  // TODO: Add your specialized code here and/or call the base class
...
CEdit* CYReportCtrl::GetEditControl() { return m_pWndEdit; }

BOOL CYReportCtrl::Create(CWnd* pParentWnd, UINT nID, LPCRECT lpRect,
                          DWORD dwStyle) {
  // TODO: Add your specialized code here and/or call the base class
...
void CYReportCtrl::PreSubclassWindow() {
  // TODO: Add your specialized code here and/or call the base class
...
BOOL CYReportCtrl::IsSortable() const {
  if (!HasColumnHeader()) return FALSE;
...
BOOL CYReportCtrl::HasColumnHeader() const {
  return (GetStyle() & LVS_NOCOLUMNHEADER) == 0;
}

void CYReportCtrl::ResizeToFitParent() {
...
BOOL CYReportCtrl::ModifyStyleEx(DWORD dwRemove, DWORD dwAdd, UINT nFlags) {
...
BOOL CYReportCtrl::ModifyStyle(DWORD dwRemove, DWORD dwAdd, UINT nFlags) {
  dwRemove &= ~MUST_STYLE;
...
void CYReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point,
                                    BOOL bTriggerEdit) {
  LVHITTESTINFO hti;
  hti.pt = point;
  ...
  const BOOL WASACTIVE =
      bTriggerEdit ? ExamItemStates(IDX, RC_ITEM_FOCUSED | RC_ITEM_SELECTED)
                   : FALSE;
  ...
  const BOOL STATSCHANGED =
      _ItemCheckMonitor(IDX, BEFORE, CListCtrl::GetCheck(IDX) > 0, nMsg);

  if (bTriggerEdit && m_bAllowEdit && !STATSCHANGED && !WAS_EDIT && WASACTIVE)
    StartEdit(IDX, hti.iSubItem);
}

BOOL CYReportCtrl::SetItemImage(int nItem, int nSubItem, int nImageIndex) {
  return CListCtrl::SetItem(nItem, nSubItem, LVIF_IMAGE, NULL, nImageIndex, 0,
                            0, 0);
}

int CYReportCtrl::GetItemImage(int nItem, int nSubItem) const {
  LVITEM lvi;
  lvi.iItem = nItem;
...
CImageList* CYReportCtrl::SetImageList(CImageList* pImageList) {
  return CListCtrl::SetImageList(pImageList, LVSIL_SMALL);
}

CImageList* CYReportCtrl::GetImageList() const {
  return CListCtrl::GetImageList(LVSIL_SMALL);
}

CImageList* CYReportCtrl::SetImageList(UINT nBitmapID, COLORREF crMask) {
  m_imgList.DeleteImageList();
  m_imgList.Create(nBitmapID, 16, 4, crMask);
  return CListCtrl::SetImageList(&m_imgList, LVSIL_SMALL);
}

BOOL CYReportCtrl::StartEdit(int nItem, int nSubItem) {
  // Get the grid width and height
  if (!m_bAllowEdit || !_IsValidIndex(nItem) || nSubItem < 0 ||
      nSubItem >= GetColumnCount())
    return FALSE;

  if (m_ptEditting.x == nItem && m_ptEditting.y == nSubItem) return TRUE;
  ...
  if (FMT != m_dwPrevEditFmt) {
    ...
    // format changes.
    if (m_pWndEdit->GetSafeHwnd() != NULL) m_pWndEdit->DestroyWindow();

    if (!m_pWndEdit->Create(
            ES_AUTOHSCROLL | ES_NOHIDESEL | WS_CHILD | WS_BORDER | FMT,
  ...
  CRect rcEdit;
  ListView_GetSubItemRect(GetSafeHwnd(), m_ptEditting.x, m_ptEditting.y,
                          LVIR_LABEL, &rcEdit);
  if (m_ptEditting.y > 0 && GetImageList() != NULL &&
      GetItemImage(m_ptEditting.x, m_ptEditting.y) >= 0)
...
BOOL CYReportCtrl::EndEdit(BOOL bCommit) {
  if (!_IsEditVisible()) return FALSE;

  CString str;
  m_pWndEdit->GetWindowText(str);
  BOOL bChanged =
      bCommit && str.Compare(GetItemText(m_ptEditting.x, m_ptEditting.y)) != 0;
  if (bChanged) {
    // update the list item
    CListCtrl::SetItemText(m_ptEditting.x, m_ptEditting.y, str);
    _UnsetSortedColumn();
    GetParent()->SendMessage(WM_EDIT_COMMITTED, (WPARAM)m_ptEditting.x,
                             (LPARAM)m_ptEditting.y);
  }
...
BOOL CYReportCtrl::SetHeaderImage(int nColumn, int nImageIndex,
                                  BOOL bLeftSide) {
  ...
  hi.mask = HDI_FORMAT;
  if (!GetHeaderCtrl()->GetItem(nColumn, &hi)) return FALSE;
...
int CYReportCtrl::GetSortedColumn() const { return m_nSortCol; }

BOOL CYReportCtrl::IsSortAscending() const { return m_bSortAscending; }

void CYReportCtrl::_UnsetSortedColumn() {
  m_nSortCol = -1;
  m_bSortAscending = TRUE;
}

DWORD CYReportCtrl::_GetHeaderTextFormat(int nColumn) const {
  if (!HasColumnHeader()) return ES_LEFT;

  HDITEM hd;
  hd.mask = HDI_FORMAT;
  if (!GetHeaderCtrl()->GetItem(nColumn, &hd)) return ES_LEFT;
...
int CYReportCtrl::GetHeaderImage(int nColumn) const {
  HDITEM hi;
  ::memset(&hi, 0, sizeof(HDITEM));
  hi.mask = HDI_IMAGE;
...
CImageList* CYReportCtrl::SetHeaderImageList(CImageList* pImageList) {
  return CListCtrl::GetHeaderCtrl()->SetImageList(pImageList);
}

CImageList* CYReportCtrl::SetHeaderImageList(UINT nBitmapID, COLORREF crMask) {
  m_headerImgList.Create(nBitmapID, 16, 4, crMask);
  return SetHeaderImageList(&m_headerImgList);
}

BOOL CYReportCtrl::ExamItemStates(int nItem, DWORD dwStates) const {
  if (dwStates == RC_ITEM_NONE) return FALSE;

  if (dwStates & RC_ITEM_ALL) return TRUE;

  return (GetItemStates(nItem) & dwStates) == dwStates;
}

int CYReportCtrl::GetItemCount(DWORD dwStates) const {
  const int ITEMS = CListCtrl::GetItemCount();
  if (dwStates == RC_ITEM_ALL) return ITEMS;

  int nCount = 0;
  for (int i = 0; i < ITEMS; i++) {
    if (ExamItemStates(i, dwStates)) nCount++;
  }
  return nCount;
}
...
void CYReportCtrl::InvertItems(int nType) {
  const int ITEMS = CListCtrl::GetItemCount();
  if (nType == RC_INVERT_SELECTION) {
    for (int i = 0; i < ITEMS; i++)
      SetItemStates(i, ExamItemStates(i, RC_ITEM_SELECTED) ? RC_ITEM_UNSELECTED
                                                           : RC_ITEM_SELECTED);
  } else if (nType == RC_INVERT_CHECKMARK) {
    for (int i = 0; i < ITEMS; i++)
      SetItemStates(i, ExamItemStates(i, RC_ITEM_CHECKED) ? RC_ITEM_UNCHECKED
                                                          : RC_ITEM_CHECKED);
  }
}

DWORD CYReportCtrl::GetItemStates(int nItem) const {
  DWORD dwStates = 0;
...
BOOL CYReportCtrl::SetItemStates(int nItem, DWORD dwNewStates) {
  BOOL bResult = _SetItemStatesNoVerify(nItem, dwNewStates);
  if (dwNewStates & RC_ITEM_CHECKED) _EnsureSingleCheck(nItem);
  return bResult;
}

int CYReportCtrl::SetAllItemStates(DWORD dwOldStates, DWORD dwNewStates) {
  if (dwNewStates == RC_ITEM_NONE || dwOldStates == RC_ITEM_NONE) return 0;
  ...
  if (dwNewStates & RC_ITEM_CHECKED) _EnsureSingleCheck(-1);

  return nCount;
}

void CYReportCtrl::_EnsureSingleCheck(int nItem) {
  if (m_nChkStyle == RC_CHKBOX_SINGLE) {
    const int FIRST = nItem < 0 ? GetFirstItem(RC_ITEM_CHECKED) : nItem;
    if (FIRST >= 0) {
      const int ITEMS = CListCtrl::GetItemCount();
      for (int i = FIRST + 1; i < ITEMS; i++) {
        if (CListCtrl::GetCheck(i)) CListCtrl::SetCheck(i, FALSE);
      }
    }
  }
}

BOOL CYReportCtrl::_SetItemStatesNoVerify(int nItem, DWORD dwNewStates) {
  if (dwNewStates == RC_ITEM_NONE) return FALSE;
...
BOOL CYReportCtrl::SetCheck(int nItem, BOOL bCheck) {
  const BOOL RES = CListCtrl::SetCheck(nItem, bCheck);
  if (RES && bCheck) _EnsureSingleCheck(nItem);
  return RES;
}

CString CYReportCtrl::GetHeaderText(int nColumn) const {
  if (!HasColumnHeader()) return _T("");
...
const CHeaderCtrl* CYReportCtrl::GetHeaderCtrl() const {
  ...
  // A LOT of const member functions. So if "GetHeaderCtrl" is not const, there
  // will be almost no const member function at all in this class. Terrible.
  return (
      const CHeaderCtrl*)(CWnd::FromHandle(ListView_GetHeader(GetSafeHwnd())));
}

void CYReportCtrl::_FreeItemMemory(int nItem) {
...
void CYReportCtrl::_AllocItemMemory(int nItem) {
  ...
  CListCtrl::SetItemData(nItem, reinterpret_cast<LPARAM>(pData));
}

void CYReportCtrl::_UpdateColumn(int nColumn, BOOL bInsert) {
...
BOOL CYReportCtrl::SetItemData(int nItem, LPARAM dwData) {
  CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
  if (p == NULL) return FALSE;
  p->dwData = dwData;
  return CListCtrl::SetItemData(nItem, reinterpret_cast<LPARAM>(p));
}

LPARAM CYReportCtrl::GetItemData(int nItem) const {
  CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
  return p == NULL ? 0 : p->dwData;
}

BOOL CYReportCtrl::GetItem(LVITEM* pItem) const {
...
BOOL CYReportCtrl::SetItem(const LVITEM* pItem) {
...
BOOL CYReportCtrl::SetItem(int nItem, int nSubItem, UINT nMask,
  ...
  return CListCtrl::SetItem(nItem, nSubItem, nMask, lpszItem, nImage, nState,
                            nStateMask, lParam);
}

void CYReportCtrl::SetItemTextColor(int nItem, int nSubItem, COLORREF color,
                                    BOOL bRedraw) {
  if (color == COLOR_INVALID) color = ::GetSysColor(COLOR_WINDOWTEXT);
  ...
  if (bRowValid && bColValid) {
    ...
    CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
    ASSERT(p != NULL);
    for (int i = 0; i < COLS; i++) p->aTextColors[i] = color;
  } else if (!bRowValid && bColValid) {
    ...
    for (int i = 0; i < ROWS; i++) {
      CItemData* p = (CItemData*)(CListCtrl::GetItemData(i));
      ASSERT(p != NULL);
      for (int j = 0; j < COLS; j++) p->aTextColors[j] = color;
    }
  }
...
void CYReportCtrl::SetItemBkColor(int nItem, int nSubItem, COLORREF color,
                                  BOOL bRedraw) {
  if (color == COLOR_INVALID) color = ::GetSysColor(COLOR_WINDOW);
  const int ROWS = GetItemCount();
  const int COLS = GetColumnCount();
  BOOL bRowValid = nItem >= 0 && nItem < ROWS;
  BOOL bColValid = nSubItem >= 0 && nSubItem < COLS;
  ...
  if (bRowValid && bColValid) {
    ...
    CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
    ASSERT(p != NULL);
    for (int i = 0; i < COLS; i++) p->aBkColors[i] = color;
  } else if (!bRowValid && bColValid) {
    // apply to whole column for all existing items
    for (int i = 0; i < ROWS; i++) {
      CItemData* p = (CItemData*)(CListCtrl::GetItemData(i));
    ...
    for (int i = 0; i < ROWS; i++) {
      CItemData* p = (CItemData*)(CListCtrl::GetItemData(i));
      ASSERT(p != NULL);
      for (int j = 0; j < COLS; j++) p->aBkColors[j] = color;
    }
  }
...
COLORREF CYReportCtrl::GetItemTextColor(int nItem, int nSubItem) const {
...
COLORREF CYReportCtrl::GetItemBkColor(int nItem, int nSubItem) const {
  if (!_IsValidIndex(nItem) || nSubItem < 0 || nSubItem >= GetColumnCount())
    return COLOR_INVALID;
  CItemData* p = (CItemData*)(CListCtrl::GetItemData(nItem));
  ASSERT(p != NULL);
...
BOOL CYReportCtrl::_PartialSort(int nStart, int nEnd) {
  ...
  for (i = 0; i < COUNT; i++) aIndices[i] = nStart + i;
...
int CYReportCtrl::_CompareItems(int nItem1, int nItem2) {
  const CString s1 = GetItemText(nItem1, m_nSortCol);
  const CString s2 = GetItemText(nItem2, m_nSortCol);

  DWORD dw1, dw2;
  if (_ITEM_COMPARE_FUNCS::_IsHexNumber(s1, dw1) &&
      _ITEM_COMPARE_FUNCS::_IsHexNumber(s2, dw2))
    return _ITEM_COMPARE_FUNCS::_HexNumberCompare(dw1, dw2);

  double f1, f2;
  if (_ITEM_COMPARE_FUNCS::_IsDecNumber(s1, f1) &&
      _ITEM_COMPARE_FUNCS::_IsDecNumber(s2, f2))
    return _ITEM_COMPARE_FUNCS::_DecNumberCompare(f1, f2);

  COleDateTime date1, date2;
  if (_ITEM_COMPARE_FUNCS::_IsDate(s1, date1) &&
...
int CYReportCtrl::_FindSeparator(int nStartAfter, int nColumn) const {
  if (m_pszSeparator == NULL) return -1;

  const int ITEMS = GetItemCount();
  for (int i = nStartAfter + 1; i < ITEMS; i++) {
    if (GetItemText(i, nColumn) == m_pszSeparator) return i;
  }
  return -1;
}

void CYReportCtrl::_QuickSortRecursive(int* pArr, int nLow, int nHigh) {
  int i = nHigh, j = nLow;
  int n = pArr[(nLow + nHigh) / 2];
  do {
    if (m_bSortAscending) {
      while (_CompareItems(pArr[j], n) < 0) j++;

      while (_CompareItems(pArr[i], n) > 0) i--;
    } else {
      while (_CompareItems(pArr[j], n) > 0) j++;
  ...
  if (nLow < i) _QuickSortRecursive(pArr, nLow, i);

  if (j < nHigh) _QuickSortRecursive(pArr, j, nHigh);
}

int CYReportCtrl::FindItem(LVFINDINFO* pFindInfo, int nStart) const {
  if (pFindInfo->flags & LVFI_PARAM) {
    for (int n = 0; n < GetItemCount(); n++) {
      CItemData* p = (CItemData*)(CListCtrl::GetItemData(n));
      if (p->dwData == pFindInfo->lParam) return n;
    }
    return -1;
  } else
