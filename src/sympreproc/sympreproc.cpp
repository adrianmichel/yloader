...
class NasdaqSymbolsList : public yloader::StrList {
 ...
 public:
  ...
  NasdaqSymbolsList(bool nyseSymbols, bool nasdaqSymbols, bool amexSymbols,
                    bool transform) {
    CurlHTTPRequest req(_T( "www.nasdaq.com" ));
    if (nasdaqSymbols) {
      std::cout << "Downoading NASDAQ symbols..." << std::endl;
  ...
  void parse(const boost::shared_ptr<std::string> str, bool transform) {
    if (!str) {
      return;
    }
    ...
    while (boost::regex_search(start, end, what, expression, flags)) {
      if (count++ >= 2) {
        ...
        if (transform) {
          ...
          if (n != std::wstring::npos) symbol.replace(n, 1, "-");

          n = symbol.find("^");
          if (n != std::wstring::npos) symbol.replace(n, 1, "-P");
...
const std::wstring NasdaqSymbolsList::nasdaqUrl =
    _T( "/screening/companies-by-industry.aspx?exchange=NASDAQ&render=download");
const std::wstring NasdaqSymbolsList::nyseUrl =
    _T( "/screening/companies-by-industry.aspx?exchange=NYSE&render=download");
const std::wstring NasdaqSymbolsList::amexUrl =
    _T( "/screening/companies-by-industry.aspx?exchange=AMEX&render=download");

class SymbolsFileWriterException {};

class SymbolsFileWriter {
  mutable std::wofstream _ofs;

 public:
  SymbolsFileWriter(const std::wstring& fileName,
                    StrListPtr symbols) throw(SymbolsFileWriterException)
      : _ofs(fileName.c_str()) {
    if (symbols) {
      std::wcout << _T( "Creating output symbols file: " ) << fileName
                 << std::endl;
      if (!_ofs) throw SymbolsFileWriterException();
...
int _tmain(int argc, _TCHAR* argv[]) {
  ...
  try {
    printDashedLine(yloader::getCaption(_T( "Symbols" )));
    printLine();
    ...
    StrListPtr sym(::getSymbols(cmdLine.symbolsFilNames(),
                                !cmdLine.allowDuplicates(), 0, &nsl));
